...
| Text run content and formatting | `replaceTextRunPlainText`<br>`setTextRunProperties`<br>`clearTextRunProperties` | S (`packages/document/src/index.ts`) | S (`packages/editor/src/index.ts`) | S (`packages/core/src/pptx-editor-session.ts#apply`) | S (`packages/editor-react/src/PptxEditor.tsx`) |
| Paragraph content and formatting | `replaceParagraphPlainText`<br>`setParagraphProperties`<br>`clearParagraphProperties` | S (`packages/document/src/index.ts`) | S (`packages/editor/src/index.ts`) | S (`packages/core/src/pptx-editor-session.ts#apply`) | I (The reusable UI edits text through run-level operations only.) |
