import { initWarningLogger } from "pptx-glimpse-renderer";
...
describe("parseTheme", () => {
  ...
  it("parses script-based font as ea fallback when ea typeface is empty", () => {
    const xml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
    <a:theme xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" name="Office Theme">
      <a:themeElements>
        <a:clrScheme name="Office">
    ...
    expect(result.fontScheme.majorFont).toBe("Arial");
    expect(result.fontScheme.majorFontEa).toBe("ＭＳ Ｐゴシック");
    expect(result.fontScheme.minorFontEa).toBe("ＭＳ Ｐゴシック");
  });

  it("uses ea typeface when it is non-empty (not falling back to script font)", () => {
    const xml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
    <a:theme xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" name="Office Theme">
      <a:themeElements>
        <a:clrScheme name="Office">
    ...
    expect(result.fontScheme.majorFontEa).toBe("メイリオ");
    expect(result.fontScheme.minorFontEa).toBe("メイリオ");
  });
